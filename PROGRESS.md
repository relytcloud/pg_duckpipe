...
- [x] Per-group config — `duckpipe.global_config` key-value table + `sync_groups.config` JSONB column. Resolution: hardcoded defaults ← global_config ← per-group JSONB. Keys: `duckdb_buffer_memory_mb`, `duckdb_flush_memory_mb`, `duckdb_threads`, `flush_interval_ms`, `flush_batch_threshold`, `max_queued_changes`. SQL API: `set_config`, `get_config`, `set_group_config`, `get_group_config`. Removed 5 GUCs (`flush_interval`, `flush_batch_threshold`, `max_queued_changes`, `duckdb_buffer_memory_mb`, `duckdb_flush_memory_mb`) and corresponding daemon CLI args.
...
- [x] Unbounded DuckDB memory — two-phase memory limits via `duckdb_buffer_memory_mb` (default 16 MB, caps in-memory buffer per table) and `duckdb_flush_memory_mb` (default 512 MB, caps DuckDB instance during flush); managed via per-group config system (`global_config` table + `sync_groups.config` JSONB)
