...
- [x] **`lag_bytes` flat during catch-up** — `send_status_update()` was not called when `poll_messages()` returned empty (no new WAL), so `pg_replication_slots.confirmed_flush_lsn` stayed frozen even as flush threads committed batches. Fixed: when no new WAL, call `coordinator.get_min_applied_lsn_in_coordinator()` and send `StandbyStatusUpdate` so the slot advances each cycle. Note: `lag_bytes = pg_current_wal_lsn() - confirmed_lsn` can still appear flat if autovacuum/checkpoints advance the write head at a similar rate to confirmed_lsn catching up — this is a metric limitation, not a code bug.
