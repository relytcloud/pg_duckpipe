...
- [ ] Flush parallelism control — global semaphore limiting how many flush threads can execute `flush_buffer()` concurrently (e.g. `duckpipe.max_concurrent_flushes`, default 4); GUC in bgworker mode, CLI arg in daemon mode; runtime-adjustable via `ALTER SYSTEM` / `pg_reload_conf()` so operators can tune without restart; remaining flush threads stay in buffer-accumulation phase (low memory) until a slot opens; also reduces DuckLake commit lock contention
