...
- [x] Parallel snapshot writes crash on concurrent COMMIT (pg_duckdb bug) — **Fixed**: replaced PG-side `INSERT INTO target SELECT * FROM source` (which went through pg_ducklake and crashed on concurrent COMMIT) with direct DuckDB connections per snapshot task via chunked COPY pipeline. Snapshots now run truly in parallel. Benchmark: multi(4x100k) improved from 13,127 rows/s (sequential) to 35,290 rows/s (parallel, 2.7x improvement).
- [ ] Multi-table streaming lag 27x higher than single-table (benchmark suite) — multi-table append avg lag=67 MB vs single-table=2.5 MB during OLTP, despite similar TPS (9,656 vs 9,509). Standalone multi-table runs show 3.7 MB avg lag (fresh DB), confirming the regression is specific to running scenarios sequentially in `bench_suite.sh`. Root cause of the stale state was the concurrent COMMIT crash during snapshot (now fixed — snapshots use direct DuckDB connections). Remaining cause: leftover flush threads from scenario 1 (cleanup removes table mappings but does not shut down the bgworker's FlushCoordinator entries for tables outside the current scenario's range). Needs re-benchmarking with the new snapshot pipeline.
